...
for itask, task in enumerate(['twotap', 'wds']):
    ...
    for name in ['Dhanashri', 'Hindol', 'Hamir', 'Gorakh', 'Gandhar']:
    ...
    for ireg, region in enumerate(['DLS', 'MC']):
        res = pickle.load(open('../results/neural_behav_corr_syn'+trimstr+'_'+region+wdsstr+'.p', 'rb'))
...
plt.savefig('../paper_figs/Sfig_trimmed.png', bbox_inches = 'tight', dpi = png_dpi)
plt.savefig('../paper_figs/Sfig_trimmed.pdf', bbox_inches = 'tight')
# plt.show()
plt.close()