...
cm = 1/2.54
# try to figure out something about latent space stability
...
#make sure the neurons actually fire in >= half the days
thresh = 0.5
thresh = 1.0
...
for i1 in range(len(early_days)): #for each day
    for i2 in range(i1+1, len(early_days)): #for each other day
        ...
        #print
        print(sims_pca[-1], sims_rho[-1], Y1.shape, Y2.shape)

#turn into numpy array
sims_cca, sims_rho, sims_pca, dts = [np.array(arr) for arr in [sims_cca, sims_rho, sims_pca, dts]]

#print the mean values as a sanity check
print('raw mean:', np.mean(sims_rho))
print('CCA mean:', np.mean(sims_cca)) #not actually an upper bound since it's greedy!!!
print('PCA mean:', np.mean(sims_pca))
...
print('CCA 95% CI:', np.quantile(alphas_c, [0.025, 0.5, 0.975]))
print('raw 95% CI:', np.quantile(alphas_r, [0.025, 0.5, 0.975]))
...
#### think about convolving and sqrt transforming!!!!
...
for d0 in range(len(early_days)):
    ...
    for d1 in range(d0+1, len(early_days)):
        ...
        print(dts[-1], np.round(m_pca1.alpha_, 2), np.round(model.alpha_, 2))

#turn into np arrays
rhos, rhos_same, dts, rhos_cca = np.array(rhos), np.array(rhos_same), np.array(dts), np.array(rhos_cca)

print('mean same day:', np.mean(rhos_same))
print('mean raw:', np.mean(rhos))
print('mean PCA:', np.mean(rhos_pca))
print('mean CCA:', np.mean(rhos_cca))
print('mean matched raw:', np.mean(rhos_sub))
