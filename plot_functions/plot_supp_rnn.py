...
for itype, lab in enumerate(labs): #for each parameter type
    ...
    if itype == 0:
        ...
        ax.set_yticks([0, 1])
        #ax.text(-0.20, 1.25, 'C', transform=ax.transAxes,fontsize=panel_font, fontweight='bold', horizontalalignment='left', verticalalignment='top')
...
# plt.text(0.51, 1.06, 'B',ha='left', va='top',transform=fig.transFigure, fontweight='bold',fontsize=panel_font)
# plt.text(-0.05, 0.48, 'C',ha='left', va='top',transform=fig.transFigure, fontweight='bold',fontsize=panel_font)

#plt.savefig('../paper_figs/Sfig_rnn.png', bbox_inches = 'tight')    
plt.savefig('../paper_figs/Sfig_rnn.png', bbox_inches = 'tight', dpi = png_dpi)
plt.savefig('../paper_figs/Sfig_rnn.pdf', bbox_inches = 'tight')
#plt.show()
plt.close()
