...
for iname, name in enumerate(['DLS', 'MC']):
    ...
    ax.plot(xs, ys, ls = '--', color = np.ones(3), lw = 1)
...
plt.savefig('../paper_figs/Sfig_by_time.png', bbox_inches = 'tight', dpi = png_dpi) 
plt.savefig('../paper_figs/Sfig_by_time.pdf', bbox_inches = 'tight')
#plt.show()
plt.close()