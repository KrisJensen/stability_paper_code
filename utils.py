...
def calc_exp_ind(dts, sims, return_res = False):
    ...
    i, s = res.x
    if (i >= global_params['min_intercept']) and res.success:
...
def load_rat(ratname, parsed = True):
    ...
    rats = []
    for name in names:
        rats.append(pickle.load(open('./data/'+name+'_data'+ext+'.p', 'rb')))       
...
def fit_asymptotic_model(ts, alphas):
    ...
    if not any([res_lin.success, res_zero.success, res_mean.success]):
        return [np.nan]*3 #discard in the unlikely event that no initialization converges
