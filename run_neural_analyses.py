...
from calc_sameday_sim import resample_rat
import sys
...
exit()
data = {}
##### run some quick firing rate analyses ####
for name in ['DLS', 'MC', 'DLS_wds', 'MC_wds']:
    print(name, 'rate')
    data[name] = {}
    rat = load_rat(name)
    unums, dts, sims, day1s = compute_neural_sim(rat, rate = True) #compute similarity vs time difference
    data[name]['unums'], data[name]['dts'], data[name]['sims'] = unums, dts, sims
    rec_times = np.array([int(np.amax(dt)+1) for dt in dts])
    data[name]['rec_times'] = rec_times
pickle.dump(data, open('./results/neural_similarity_analyses_rates.p', 'wb'))