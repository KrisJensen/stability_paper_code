...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep  7 22:23:44 2021
...
"""
import sys
if sys.platform == 'linux': #if we're on the cluster, don't open figures
    import matplotlib
    matplotlib.use('Agg')
...
def gen_obs(x, y, kernel = 15, scale_rate = 0.1, max_fr = None):
    ...
    if max_fr is not None:
        print('soft capping firing rates')
...
def run_cca(Xa, Xb, nPC = 10, nCC = 4, return_aligned = False):
    ...
    if return_aligned:
        ...
        #CCA correlation; average across the nCC top dimensions
        cor_CC = np.mean([pearsonr(La[i, :], Lb_aligned[i, :])[0] for i in range(nCC)])
...
def plot_rnn(n_train_steps, cost_over_epochs, x_out_f, y_np, fname = 'none', W_sims = [],
             w_out_sims = [], labs = ['target', 'simulation']):
    
    #plot results
    plt.figure(figsize = (5,3.5))
    plt.plot(range(n_train_steps), cost_over_epochs); plt.yscale('log')
    plt.xlabel('epoch')
    plt.ylabel('log error')
    if not fname == 'none': plt.savefig('./plots/'+fname+'_loss.pdf', dpi = 240)
    if sys.platform != 'linux': plt.show()
    plt.close()
    
    plt.figure(figsize = (5,3.5))
    cs = np.linspace(0.5, 1, y_np.shape[0])
    for i in range(y_np.shape[0]):
        plt.plot(y_np[i, :], c = [0, 0, cs[i]], ls = '-')
        plt.plot(x_out_f[i, :], c = [0, cs[i], 0], ls = '-') 
    plt.xlabel('time in trial')
    plt.ylabel('position')
    plt.legend(labs)
    if not fname == 'none': plt.savefig('./plots/'+fname+'_output.pdf', dpi = 240)
    if sys.platform != 'linux': plt.show()
    plt.close()
    
    if max(len(W_sims), len(w_out_sims)) > 0:
        plt.figure(figsize = (5, 3.5))
        legend = []
        if len(W_sims) > 0:
            W_sims = W_sims[w_out_sims > 0]
            plt.plot(range(len(W_sims)), W_sims, 'k-')
            legend.append('recurrent weights')
        if len(w_out_sims) > 0:
            w_out_sims = w_out_sims[w_out_sims > 0]
            plt.plot(range(len(w_out_sims)), w_out_sims, 'b-')
            legend.append('readout weights')
            
        plt.legend(legend)
        plt.xlabel('epoch')
        plt.ylabel('weight correlation')
        if not fname == 'none': plt.savefig('./plots/'+fname+'_weight_change.pdf', dpi = 240)
        if sys.platform != 'linux': plt.show()
        plt.close()
...
def interp_mats(Ws, x_ics, w_outs, bs, y_np, n_train_steps, dt, tau, noise, reg = 1e-3, ninterp = 5, lrate = 50e-6):
    ...
    for i, W in enumerate(W_interp):
        ...
        print(np.mean([pearsonr(xs[j,:], xs_interp[0][j,:])[0] for j in range(N)]))
