...
def parse_behav(rat, maxnan = 0.5):
    ...
    for day in rat['trials'].keys():
        ...
        for limb in rat['trials'][day]['kinematics'].keys():
            ...
            ncoord = rat['trials'][day]['kinematics'][limb].shape[-1]
            #print(rat['trials'][day]['kinematics'][limb].shape)
            print(rat['trials'][day]['kinematics'][limb].shape)
    ...
    for unum in rat['units'].keys():
        ...
        for day in rat['units'][unum].keys():
            ...
            for itrial in range(len(keep)):
                ...
                start = 0#np.searchsorted(spikes, tmin)
                end = len(spikes)# np.searchsorted(spikes, (tmax-tint)+Int) #only include tmax+Int-700 ms after second tap
...
def select_trials(rat, all_keep):
    ...
    for day in all_keep.keys():
        ...
        #print('\n', rat['trials'][day]['kinematics']['paw_L'][0][:10, :], '\n', rat['trials'][day]['kinematics']['paw_L'][-1][:10, :])
...
def calc_peth(raster, tmin, tmax, wds = False, gauss = True):
    ...
    if global_params['smooth_peth'] and gauss:
        ...
        #assert len(peth_c) == len(peth)
        #print(sum(peth), sum(peth_new), sum(peth_c))
        peth = peth_c
...
def subselect_trials(rat, mode = 'default', ipi_min = 'default', ipi_max = 'default', mintrial = 10):
    ...
    if mode == 'default':
        mode = helper_functions.get_modes(rat['name'], predominant = True)[0]
    ...
    for day in rat['trials'].keys():
        ...
        #print([[k, type(rat['trials'][day][k])] for k in rat['trials'][day].keys()])
...
def keep_warped(rat):
    for unum, u in rat['units'].items(): #for each unit
        for day in u.keys():
            ...
            #del rat['units'][unum][day]['raster_w']
