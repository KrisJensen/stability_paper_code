...
for rep in reps:
    ...
    print('mean rates:', [np.mean(x) for x in xs])
    peths, vels, spikes, spikes_c = np.array(peths), np.array(vels), np.array(spikes), np.array(spikes_c)
    print('peth, vel, spike shape:', peths.shape, vels.shape, spikes.shape)
    ...
    plt.figure()
    plt.plot(bin_dts, m, 'k-')
    plt.fill_between(bin_dts, m-s, m+s, color = 'k', alpha = 0.2)
    plt.savefig('./plots/rnn/test_behav'+driftstr+'.png')
    plt.close()
    ...
    plt.plot(bin_dts, m, 'k-')
    plt.fill_between(bin_dts, m-s, m+s, color = 'k', alpha = 0.2)
    plt.savefig('./plots/rnn/test_neural'+driftstr+'.png')
    plt.close()


    ### compute stability indices ###
    alphas, _ = calc_alphas(np.arange(N), dts, sims, comp_error = False)
    print(len(alphas), -1/np.quantile(alphas, [0.25, 0.5, 0.75]))
    print(alphas[:5])
    data[rep]['alphas'] = alphas
        
    if rep == 0:
        ### plot example rasters for stable and unstable units!
        ex_spikes = []
        for n in ex_ns[:3]:
            yval = 0
            plt.figure(figsize = (4, 6))
            for i in range(ninterp):
                col = [0, 0.1+0.8*i/ninterp, 0.9-0.8*i/ninterp]
                for t in range(0, ntrials, 1):
                    rast = spikes[i, t, n, :]
                    rast = np.nonzero(rast)[0]
                    yval -= 1
                    plt.scatter(rast, np.ones(len(rast))*yval, color = col, s = 3, alpha = 0.3)
                    #print(len(rast))
                
        plt.savefig('./plots/rnn/test_raster'+str(n)+driftstr+'.png')
        plt.close()
    ...
    plt.plot(bin_dts, m, 'k-')
    plt.fill_between(bin_dts, m-s, m+s, color = 'k', alpha = 0.2)
    plt.savefig('./plots/rnn/test_CCA'+driftstr+'.png')
    plt.close()
