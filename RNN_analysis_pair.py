...
print(x1.shape, peth1.shape, spikes1.shape, spikes_c1.shape)

print('plot raw data')
plt.figure()
plt.plot(x1[0, 0, :], 'r-')
plt.plot(x2[0, 0, :], 'b-')
plt.plot(ts, spikes_c1[0, 0, :]/scale_rate, 'm-')
plt.plot(ts, peth1[0, :]/ntrial/scale_rate, 'k-')
plt.savefig('plots/rnn/testx.png')
plt.close()

plt.figure()
plt.plot(y1[0, 0, :], 'r-')
plt.plot(y2[0, 0, :], 'b-')
plt.plot(ts, vel1[0, :], 'k-')
plt.savefig('plots/rnn/testy.png')
plt.close()
...
plt.figure()
plt.plot(rs[1:], c_same, 'c-')
plt.plot(rs[1:], c_diff, 'm-')
plt.axvline(np.sum(rs[1:]*c_same/sum(c_same)), color = 'c', ls = ':')
plt.axvline(np.sum(rs[1:]*c_diff/sum(c_diff)), color = 'm', ls = ':')
plt.xlim(-1, 1)
plt.savefig('plots/rnn/test_corr.png')
plt.close()
print(sum(c_same), sum(c_diff))
...
for n in range(nrep):
    if n % 10 == 0 and n > 0:
        print(n, np.mean(CCs[:n, :], axis = 0), np.std(CCs[:n, :], axis = 0))
...
plt.axvline(np.sum(rs[1:]*c_diff/sum(c_diff)), color = 'm', ls = ':')
plt.xlim(-1, 1)
plt.savefig('plots/rnn/test_CC.png')
plt.close()
print(sum(c_same), sum(c_diff))
