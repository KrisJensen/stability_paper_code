...
if np.amax(subsamps) == 1:
    ss = np.array([0.2, 0.2, 0.10, 0.2])/1
    alphas = np.array([0.10, 0.2, 0.04, 0.2])
else:
...
for name in ['MC', 'DLS']:
    ...
    inds = (rec_times >= 14).nonzero()[0]
    bins = np.arange(0.5, 14.5+0.01, 1)
    long_sims = np.zeros((len(inds), len(bins)-1))
    for iind, ind in enumerate(inds):
        dt, sim = dts[ind], sims[ind]
        long_sims[iind, :] = binned_statistic(dt, sim, statistic = 'mean', bins = bins)[0]
...
#ax.axhline(y=alpha_d, color='magenta', ls=':')
#ax.axhline(y=alpha_s, color='cyan', ls=':')
...
for ireg, region in enumerate(['DLS', 'MC']):
    ...
    if ireg == 0:
        ax.set_ylabel(r'$\alpha$', labelpad = -20)
        ax.set_yticks([-0.05, 0.0])
        #ax.text(-0.1, 1.05, 'G', transform=ax.transAxes,fontsize=panel_font, fontweight='bold', horizontalalignment='right', verticalalignment='bottom')
    else:
        ax.set_yticks([])
    ax.set_ylim(-0.06, 0.005)
...
#plt.savefig('../paper_figs/fig_neural.png', bbox_inches = 'tight')
plt.savefig('../paper_figs/fig_neural.png', bbox_inches = 'tight', dpi = png_dpi)
plt.savefig('../paper_figs/fig_neural.pdf', bbox_inches = 'tight')
# plt.show()
plt.close()
