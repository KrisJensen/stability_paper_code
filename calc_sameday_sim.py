...
#go through every animal
#go through every session
#concatenate all sessions with >=minspike spikes
#resample all sessions with replacement
#if >= minspike spikes: compute similarity with replacement
...
def resample_sessions(rat, nsample = 1000):
    ...
    for u in us:
        ...
        print(mean_sims[u])
    means = [np.nanmean(mean_sims[u]) for u in us]
...
def resample_rat(rat):
    ...
    for unum, unit in rat['units'].items():
        ...
        if len(days) > 0:
            ...
            # big_peth = np.concatenate([unit[day]['peth_w_t'] for day in days])
            # assert ntrial == big_peth.shape[0]
            ...
            for iday, day in enumerate(days):
                ...
                # peth_w_t = big_peth[inds, :]
                # peth_w = np.sum(peth_w_t, axis = 0) #is this wrong?
...
if __name__ == '__main__':
    name = 'Hindol'
    rat = load_rat(name)
    all_sims, mean_sims = resample_sessions(rat)
