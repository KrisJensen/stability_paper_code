...
import numpy as np
import sys
#if we're on the cluster, don't open figures
...
from RNN_utils import sim_rnn_single, plot_rnn, interp_mats
...
for rep in reps:
    ...
    for i in range(nout):
        plt.plot(ts, y_np[i, :])
    plt.savefig('plots/RNN_test_output.png')
    plt.close()

    print('simulating network with', N, 'recurrent neurons and', nout, 'readout neurons.\n\n')

    print('N', N, 'T', T, 'dt', dt, 'tau', tau, 'n_train_steps', n_train_steps, 'noise', noise)
    ...
    if from_pickled:
        ...
        for i in range(2):
            ...
            plot_rnn(new_train_steps, cost_over_epochs, x_out, y_np, W_sims = W_sims, w_out_sims = w_out_sims, fname = 'rnn/network'+str(i))

            #verify that internal dynamics have actually changed
            plt.hist([scipy.stats.pearsonr(xs[j,:], xs_i[j,:])[0] for j in range(N)])
            if sys.platform != 'linux': plt.show()
            plt.close()
            print('mean cor:', np.mean([scipy.stats.pearsonr(xs[j,:], xs_i[j,:])[0] for j in range(N)]))
            print(np.std(W), np.mean(xs), np.std(np.mean(xs, axis = 1)))
